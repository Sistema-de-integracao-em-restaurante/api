...
def test_prato_get_by_id(client, session_scope):
    ...
    prato_to_search = Prato(id=1, nome="Prato", preco=30.5)
...
def test_prato_set(client, session_scope):
    ...
    response = client.post("/api/prato", json={"nome": "Prato", "preco": 20.2})
...
def test_prato_set_withoud_required(client):
    response = client.post("/api/prato", json={})

    assert response.status_code == 400


def test_prato_set_withoud_required_2(client):
    response = client.post("/api/prato", json={"nome": "Prato1"})
...
def test_prato_delete(client, session_scope):
    ...
    prato_to_delete = Prato(id=1, nome="Prato", preco=26.4)
    ...
    session.query().filter().first.assert_called_once()
    session.commit.assert_called_once()
    session.delete.assert_has_calls([mock.call(), mock.call(prato_to_delete)])
    assert response.status_code == 200
    assert response.json["nome"] == "Prato"
